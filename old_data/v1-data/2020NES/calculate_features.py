...
def calculate_temporal_features(utterances):
    """Calculate temporal and sequential metrics."""
    features = {}
    
    # Split utterances into first and second half
    mid_point = len(utterances) // 2
    first_half = utterances[:mid_point]
    second_half = utterances[mid_point:]
    
    # Count new ideas in each half
    first_half_ideas = sum(1 for u in first_half if 'present new idea' in u.get('annotations', {}))
    second_half_ideas = sum(1 for u in second_half if 'present new idea' in u.get('annotations', {}))
    
    features['ideas_first_half'] = first_half_ideas
    features['ideas_second_half'] = second_half_ideas
    
    # Calculate decision lag
    decision_lags = []
        ...
        if 'present new idea' in utterance.get('annotations', {}):
            ...
            for j in range(i + 1, len(utterances)):
                if 'propose decision' in utterances[j].get('annotations', {}):
                    decision_lags.append(j - i)
                    break
    
    if decision_lags:
        features['decision_lag'] = np.mean(decision_lags)
    
    # Calculate idea-agree-decision chain count
    chain_count = 0
    ...
    features['idea_agree_decision_chain_count'] = chain_count
    
    # Calculate burstiness of ideas
    ...
    # Calculate conflict resolution latency
        ...
        if 'reject idea' in utterance.get('annotations', {}):
            ...
            for j in range(i + 1, len(utterances)):
                if 'resolve conflict' in utterances[j].get('annotations', {}):
                    resolution_latencies.append(j - i)
                    break
...
def process_session_file(file_path):
    ...
    utterances = data.get('all data', [])  # Changed from 'utterances' to 'all data'
...
def main():
    ...
    output_dir = 'gemini_code/data/2020-NES/featurized data'
    os.makedirs(output_dir, exist_ok=True)
    
    # Process all JSON files in the directory
    data_dir = 'gemini_code/data/2020-NES'
    for filename in os.listdir(data_dir):
        if filename.endswith('.json') and filename != 'outcome.json':
